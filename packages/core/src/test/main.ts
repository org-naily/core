/*
 * @Author: Zero的Mac 1203970284@qq.com
 * @Date: 2023-09-24 22:50:21
 * @LastEditors: Zero的Mac 1203970284@qq.com
 * @LastEditTime: 2023-09-26 09:07:50
 * @FilePath: /v5/packages/core/src/test/main.ts
 * @Description: 这是默认设置,请设置`customMade`, 打开koroFileHeader查看配置 进行设置: https://github.com/OBKoro1/koro1FileHeader/wiki/%E9%85%8D%E7%BD%AE
 */

import { Configuration, Controller, Get, IConfigurationBooter, Injectable, Post, onInit, Value, onMounted } from "..";
import { ExpressAdapter } from "./adapter/express.adapter";

@Injectable
export class TestService implements onMounted {
  @Value("typeorm.password")
  private readonly password: number;

  handleMounted(): void | Promise<void> {
    console.log(this.password);
  }
}

@Controller()
export class TestController {
  @Value("typeorm.password")
  private readonly password: number;

  @Get()
  @Post()
  getHello() {
    return this.password.toString();
  }
}

@Configuration({ controllerAdapter: ExpressAdapter })
export class BootStrap implements IConfigurationBooter {
  main(): number {
    return 4000;
  }
}
